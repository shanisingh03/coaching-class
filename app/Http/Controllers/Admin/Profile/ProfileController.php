...
class ProfileController extends Controller
{
    ...
    public function getProfileDetail()
    {
        $user = Auth::user();
        $activeTab = 'profile';
        return view('admin.profile.profile')->with(['user'=>$user,'activeTab'=>$activeTab]);
    }
    ...
    public function updateProfile(UpdateProfileRequest $request)
    {
        ...
        try {
            ...
            $user = Auth::user();
            DB::commit();
            return redirect()->route('admin.profile.details')->with(['user'=>$user,'activeTab'=>$activeTab,'success'=>'Profile Data updated successfully.']);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function resetPassword(ChangePasswordRequest $request)
    {
        ...
        try {
            ...
            $user = Auth::user();
            DB::commit();
            return redirect()->route('admin.profile.details')->with(['user'=>$user,'activeTab'=>$activeTab,'success'=>'Password Changed successfully.']);
        } catch (\Exception $e) {
            DB::rollback();
            return back()->withInput()->with(['activeTab'=>$activeTab,'error'=>$e->getMessage()]);
        }
